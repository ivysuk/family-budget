...
// -------------------------------------------------------------------------
// 예금이자 (수동 입력) — 누가 낸 돈이 아니라 은행이 넣어준 돈이라, 월별납부(멤버별 목표달성 현황)엔
// 안 잡히고 거래원장(잔액 계산)에만 반영한다. 이자는 매번 대표님이 통장 보고 직접 입력.
// -------------------------------------------------------------------------

function recordInterest(amount, memo) {
  if (!amount) throw new Error('금액은 필수입니다.');
  const today = new Date();
  postLedger(today, '공동통장', '예금이자', Number(amount), '', memo || '예금이자', '이자', '');
  return { ok: true };
}
